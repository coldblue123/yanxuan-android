...
public class Find_index extends Activity {
	...
	public void initdata() {
		...
		list=Goods.getListToHashMap(Goods.selectGoodsByNameOrClass(q.GoodsList,findEditStr));
		//Toast.makeText(getApplicationContext(), "数组："+findEditStr+"|"+Goods.selectGoodsByNameOrClass(q.GoodsList,findEditStr).size(), 1).show();
	}
	...
	public void setSimple() {
		...
		(       Find_index.this, // 当前类
				...
				new String[] { "Image", "UintName", "Price" , "Image", "UintName", "Price" }, // 字符串数组，里面放参数名。
				new int[] { R.id.imageView1,  R.id.textView1,R.id.imageView2} // int数组，里面放数据的控件id，位置要与参数名一一对应。
		) /*{
			...
			public View getView(final int position, View convertView, ViewGroup parent) {
				...
				Button bt1 = (Button)view.findViewById(R.id.button1); // 找view的button1
				final TextView tv = (TextView)view.findViewById(R.id.text2); // 找view的button1
				bt1.setOnClickListener(new OnClickListener() {
					@Override
					public void onClick(View v) {
						if (position==0) {
							Toast.makeText(getApplicationContext(), "第一列菜数目加一", 1)
							.show();
						}
						if (position==1) {
							Toast.makeText(getApplicationContext(), tv.getText(), 1)
							.show();
							tv.setText("红烧肉");
						}
		...
		ListView lv = (ListView) findViewById(R.id.linearLayout2); // 找ListView控件
		lv.setAdapter(adapter); // 把适配器设置给ListView控件
